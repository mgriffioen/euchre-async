 ...
 * - UI shows Team A = NS and Team B = EW (does not flip per viewer)
 */
type TeamKey = "NS" | "EW";
...
/**
 * ==========================================================
 * Small UI Components (presentational)
 * ==========================================================
 */
  ...
  return (
    <div
      style={{
        ...
        paddingTop: 6,
        paddingBottom: 10,
        minHeight: 130, // keeps boxes consistent
      }}
    >
      {/* Main content */}
      <div
      ...
      >
        ...
        >
          <div style={{ color: "#555", fontSize: 18, lineHeight: 1.15 }}>
            {label}
          </div>
...
export default function Game() {
   ...
   * - Display rotation is applied later for UI only
   */
   ...
   * ----------------------------------------------------------
   *
   * DISPLAY seats are used only for UI.
   * Never write DISPLAY seats to Firestore; always write REAL seats.
   */
   ...
   *
   * Notes:
   * - These functions write to Firestore (transactions/batches).
   * - Seat rotation is UI-only; all writes use REAL seats.
   */

  // Writes: game.seats + players/{uid} metadata
  ...
  // Writes: game doc + all four players' hands (batch)
  ...
  // Writes: game doc transaction (bidding state)
  ...
  // Writes: game doc transaction (bidding → dealer discard)
  ...
  // Writes: game doc transaction (bidding → playing)
  ...
  // Writes: dealer player hand + game doc transaction (pickup/discard)
  ...
  // Writes: player hand + game doc transaction (trick progression)
  async function playCard(code: CardCode) {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        // - uses effectiveSuit() so the left bower counts as trump for suit-following
        // - only enforced when not leading
        if (!isNewTrick) {
        ...
        // Trick not complete yet → next seat (clockwise)
        tx.update(playerRef, { hand: nextHand, updatedAt: serverTimestamp() });
      ...
      {!game ? (
        ...
        <>
          ...
          <div style={cardStyle}>
            ...
            <div>
              <b>Score:</b> {teamUi.labelForTeam[teamUi.aTeam]} {game.score[teamUi.aTeam]} —{" "}
              {teamUi.labelForTeam[teamUi.bTeam]} {game.score[teamUi.bTeam]}
            </div>
